...
public class SwaggerConfigurationProvider {
        ...
        private static final String TITLE = "API測試";
        private static final String DESCRIPTION = "先透過  /oauth/token 取得 access_token 再將 Token 放到 Request header 的 Authorization: bearer ${access_token} 再操作 新增跟取得";
        ...
        private List<Response> getGlobalResonseMessage() {
                ...
                responseList.add(this.createResponse(HttpStatus.BAD_REQUEST, HttpStatus.BAD_REQUEST.getReasonPhrase()));
                responseList.add(this.createResponse(HttpStatus.UNAUTHORIZED,
                                HttpStatus.UNAUTHORIZED.getReasonPhrase()));
                responseList.add(this.createResponse(HttpStatus.FORBIDDEN, HttpStatus.FORBIDDEN.getReasonPhrase()));
                responseList.add(this.createResponse(HttpStatus.NOT_FOUND, HttpStatus.NOT_FOUND.getReasonPhrase()));
                responseList.add(this.createResponse(HttpStatus.CONFLICT, HttpStatus.CONFLICT.getReasonPhrase()));
        ...
        private Response createResponse(HttpStatus httpStatus, String description) {
                ...
                return new ResponseBuilder()
                        ...
                        .apply(pepresentationBuilder -> pepresentationBuilder
                                .model(modelSpecificationBuilder -> modelSpecificationBuilder
                                        .name("ErrorResponse")
                                        .referenceModel(referenceModelSpecificationBuilder -> referenceModelSpecificationBuilder
                                                .key(modelKeyBuilder -> modelKeyBuilder
                                                        .isResponse(true)
                                                        .qualifiedModelName(qualifiedModelNameBuilder -> qualifiedModelNameBuilder
                                                                .name("ErrorMsg")
                                                                .namespace("com.example.demo.api.model")
                                                                .build()
                                                        ).build()
                                                ).build()
                                        ).build()
        ...
        private List<SecurityScheme> securitySchemes() {
                ...
                OAuth oAuth = new OAuthBuilder().name(BEARER_TOKEN_NAME).grantTypes(Collections.singletonList(grantType))
        ...
        private AuthorizationScope[] scopes() {
                ...
                scopeList.add(new AuthorizationScope("calendar", "See, edit, share, and permanently delete all the calendars you can access using Calendar"));
                scopeList.add(new AuthorizationScope("calendar.readonly", "View your calendars"));
                scopeList.add(new AuthorizationScope("calendar.events", "View and edit events on all your calendars"));
                scopeList.add(new AuthorizationScope("calendar.events.readonly", "View events on all your calendars"));
        ...
        private SecurityContext securityContextLogin() {
                return new SecurityContextBuilder().securityReferences(
                                Collections.singletonList(new SecurityReference.SecurityReferenceBuilder()
                                                .reference(BASIC_AUTH_NAME).scopes(new AuthorizationScope[0]).build()))
        ...
        private SecurityContext createSecurityContext(String securityReferenceName, String uri,
                        RequestMethod requestMethod) {
                return new SecurityContextBuilder()
                                .securityReferences(Collections
                                                .singletonList(new SecurityReference.SecurityReferenceBuilder()
                                                                .reference(securityReferenceName)
                                                                .scopes(new AuthorizationScope[0]).build()))
                                .operationSelector(operationContext -> operationContext.requestMappingPattern().equals(
                                                uri) && operationContext.httpMethod().equals(requestMethod.toString()))
                                .build();
        }
